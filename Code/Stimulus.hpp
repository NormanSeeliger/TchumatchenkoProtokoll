...
public:
    ...
    * - return: whether suceeded or not */
	bool printShape(const char* name, double dt) const
	{
		...
		// write script file
		f << "set term pdf enhanced font \"FrontPage, 9\" color solid lw 3" << endl;
    ...
    * - return: true if successful */
	bool addSinePulse(double magnitude, int offset, int period)
	{
		...
		{
			shape[offset+i] += magnitude * sin( (2.0*3.1415/period) * i );
		}
		return true;
	}
    ...
    * and copies them into a new Stimulus (basically does *
	...
	{
		freeShape();
		//cout << "Destructor ~Stimulus() (n=" << n << ") was called!" << endl;
	}
